...
const SERVICE_GUIDANCE: Record<ServiceId, string> = {
  ...
  verify:
    "If the user pasted claims, review them one by one. If the user asks a QUESTION (e.g. 'is X true?'), answer it directly using the SOURCES — lead with a clear one-sentence conclusion in `summary`, then a 'Findings' section explaining what the sources show. Record each supporting or contradicting point as a `claims` entry: 'verified_fact' (supported by a cited source + excerpt), 'reported_claim', 'opinion', 'not_sufficiently_supported', or 'unresolved_question' when the sources don't settle it. Do NOT answer from memory — if no source supports a point, mark it unresolved rather than guessing. Add a short note that this is based on the sources Harbor could access.",
  compare:
